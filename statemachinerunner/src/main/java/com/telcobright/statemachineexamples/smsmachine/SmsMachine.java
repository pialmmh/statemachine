 ...
 */
public class SmsMachine extends GenericStateMachine<SmsContext> {
    ...
    public SmsMachine(String id, StateMachineSnapshotRepository snapshotRepository, 
        ...
        System.out.println("🔧 Created SmsMachine: " + id);
    }
    
    /**
     * Create and configure the SMS state machine using generic fluent builder
     */
    public static SmsMachine create(String machineId) {
        // Using the generic FluentStateMachineBuilder - no custom builder needed!
        return (SmsMachine) FluentStateMachineBuilder.<SmsContext>create(machineId)
            ...
            // SENDING state - SMS being transmitted
            .state(SmsMachineState.SENDING)
                ...
                .stay(StatusUpdate.class, (machine, event) -> OnStatusUpdate_SENDING.handle((GenericStateMachine<SmsContext>) machine, (StateMachineEvent) event))
                .done()
    ...
    public static Boolean saveSmsMachineSnapshot(Connection conn, StateMachineSnapshotEntity snapshot) {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            System.out.println("💾 Saved smsmachine snapshot: " + snapshot.getMachineId() + " in state " + snapshot.getStateId());
            return rowsAffected > 0;
            
        } catch (SQLException e) {
            System.err.println("❌ Failed to save smsmachine snapshot: " + e.getMessage());
            return false;
        }
    }
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    System.out.println("📖 Loaded smsmachine snapshot: " + machineId + " in state " + snapshot.getStateId());
                    return snapshot;
                }
            }
        } catch (SQLException e) {
            System.err.println("❌ Failed to load smsmachine snapshot: " + e.getMessage());
        }
    ...
    public static Boolean initSmsMachineTable(Connection conn) {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(createTableSql)) {
            pstmt.execute();
            System.out.println("🗄️ SmsMachine snapshots table initialized");
            return true;
        } catch (SQLException e) {
            System.err.println("❌ Failed to initialize smsmachine table: " + e.getMessage());
            return false;
        }
    }
