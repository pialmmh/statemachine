...
public class {{machineName}} {
    ...
    public GenericStateMachine createMachine(String machineId) {
        ...
        System.out.println("🔧 Creating {{machineName}} with profile: " + activeProfile);
    ...
    public static Boolean save{{machineName}}Snapshot(Connection conn, StateMachineSnapshotEntity snapshot) {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            System.out.println("💾 Saved {{lowercase machineName}} snapshot: " + snapshot.getMachineId() + " in state " + snapshot.getStateId());
            return rowsAffected > 0;
            
        } catch (SQLException e) {
            System.err.println("❌ Failed to save {{lowercase machineName}} snapshot: " + e.getMessage());
            return false;
        }
    }
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    System.out.println("📖 Loaded {{lowercase machineName}} snapshot: " + machineId + " in state " + snapshot.getStateId());
                    return snapshot;
                }
            }
        } catch (SQLException e) {
            System.err.println("❌ Failed to load {{lowercase machineName}} snapshot: " + e.getMessage());
        }
    ...
    public static Boolean init{{machineName}}Table(Connection conn) {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(createTableSql)) {
            pstmt.execute();
            System.out.println("🗄️ {{machineName}} snapshots table initialized");
            return true;
        } catch (SQLException e) {
            System.err.println("❌ Failed to initialize {{lowercase machineName}} table: " + e.getMessage());
            return false;
        }
    }
    {{/if}}
...
async function generateFiles(dsl, outputDir) {
  ...
  console.log(`✅ Generated ${dsl.machineName} state machine in ${baseDir}`);
}
...
program
  ...
  .action(async (options) => {
    try {
      console.log('🚀 State Machine Scaffolder\n');
      ...
      console.log(`📖 Parsed specification for ${dsl.machineName}`);
...
public class ${options.name}Scaffolder {
    ...
    console.log(`✅ Created specification template: ${options.output}`);
    console.log(`\n📝 Edit the specification and then run:`);
