...
public class StateMachineWebSocketServer extends WebSocketServer 
    ...
    private void registerEventTypes() {
        com.telcobright.statemachine.events.EventTypeRegistry.register(
            com.telcobright.examples.callmachine.events.IncomingCall.class, "INCOMING_CALL");
        com.telcobright.statemachine.events.EventTypeRegistry.register(
            com.telcobright.examples.callmachine.events.Answer.class, "ANSWER");
        com.telcobright.statemachine.events.EventTypeRegistry.register(
            com.telcobright.examples.callmachine.events.Hangup.class, "HANGUP");
        com.telcobright.statemachine.events.EventTypeRegistry.register(
            com.telcobright.examples.callmachine.events.Reject.class, "REJECT");
        com.telcobright.statemachine.events.EventTypeRegistry.register(
            com.telcobright.examples.callmachine.events.SessionProgress.class, "SESSION_PROGRESS");
    }
    ...
    public void onMessage(WebSocket conn, String message) {
        try {
            ...
            System.out.println("[WS] Full message: " + message.substring(0, Math.min(message.length(), 200)));
    ...
    private void routeEventToMachine(String machineId, String eventType, JsonObject payload) {
        ...
        if (machine == null && registry instanceof StateMachineRegistry) {
            ...
            if (machineId.startsWith("call-")) {
                ...
                try {
                    com.telcobright.debugger.CallMachineRunnerEnhanced.CallPersistentContext context = 
                        typedRegistry.rehydrateMachine(
                            machineId,
                            com.telcobright.debugger.CallMachineRunnerEnhanced.CallPersistentContext.class,
                            () -> {
                                // Create new context for machines that don't exist in persistence
                                // Using dummy caller/callee for rehydration test
                                return new com.telcobright.debugger.CallMachineRunnerEnhanced.CallPersistentContext(
                                    machineId,
                            ...
                            (ctx) -> {
                                ...
                                GenericStateMachine<com.telcobright.debugger.CallMachineRunnerEnhanced.CallPersistentContext, ?> typedMachine = 
                                    (GenericStateMachine<com.telcobright.debugger.CallMachineRunnerEnhanced.CallPersistentContext, ?>) 
            ...
            try {
                ...
                if (eventType.equals("INCOMING_CALL")) {
                    String phoneNumber = payload != null && payload.has("phoneNumber") 
                        ...
                        : "+1-555-0000";
                    event = new com.telcobright.examples.callmachine.events.IncomingCall(phoneNumber, "+1-555-9999");
                } else if (eventType.equals("ANSWER")) {
                    event = new com.telcobright.examples.callmachine.events.Answer();
                } else if (eventType.equals("HANGUP")) {
                    event = new com.telcobright.examples.callmachine.events.Hangup();
                    ...
                    int ringNumber = payload != null && payload.has("ringNumber") 
                        ...
                        : 1;
                    event = new com.telcobright.examples.callmachine.events.SessionProgress(sessionData + " (ring " + ringNumber + ")");
                } else if (eventType.equals("REJECT")) {
                    String reason = payload != null && payload.has("reason") 
                        ? payload.get("reason").getAsString() 
                        : "User Busy";
                    event = new com.telcobright.examples.callmachine.events.Reject(reason);
                }
                ...
                if (event != null) {
                    ...
                    System.out.println("[WS] About to fire event: " + event.getClass().getName() + 
                                     " on machine: " + machineId + " in state: " + oldState);
                    System.out.println("[WS] Event details: " + event.toString());
                    
                    // Fire the strongly typed event to the state machine
                    try {
                        machine.fire(event);
                    } catch (Exception e) {
                        System.out.println("[WS] Error firing event: " + e.getMessage());
                        e.printStackTrace();
                    }
    ...
    private GenericStateMachine<?, ?> createCallMachineForRehydration(String machineId) {
        ...
        class RehydrationContext implements StateMachineContextEntity<String> {
            private String currentState = CallState.ADMISSION.name();
        ...
        return FluentStateMachineBuilder.<RehydrationContext, Void>create(machineId)
            .initialState(CallState.ADMISSION)
            
            .state(CallState.ADMISSION)
                .on(com.telcobright.examples.callmachine.events.IncomingCall.class)
                    .to(CallState.RINGING)
                .done()
                
            .state(CallState.RINGING)
                .timeout(30, TimeUnit.SECONDS, CallState.ADMISSION.name())
                .on(com.telcobright.examples.callmachine.events.Answer.class)
                    .to(CallState.CONNECTED)
                .on(com.telcobright.examples.callmachine.events.Hangup.class)
                    .to(CallState.ADMISSION)
                .on(com.telcobright.examples.callmachine.events.Reject.class)
                    .to(CallState.ADMISSION)
                .stay(com.telcobright.examples.callmachine.events.SessionProgress.class, (machine, event) -> {
            ...
            .state(CallState.CONNECTED)
                ...
                .timeout(30, TimeUnit.SECONDS, CallState.ADMISSION.name())  // Match CallMachineRunnerProper's 30 second timeout
                .on(com.telcobright.examples.callmachine.events.Hangup.class)
                    .to(CallState.ADMISSION)
                .done()
